...
#'   of the base learners. Use \code{"best"} to act as a Discrete Super Learner, which
#'   assigns a weight of 1.0 to the single model with the lowest cross-validated risk.
...
SuperSurv <- function(time, event, X, newX = NULL, new.times,
  ...
  # ----------------------------------------------------------------------------
  # 8b. 'Best' Model Selection Logic (Winner-Take-All Override)
  # ----------------------------------------------------------------------------
  if (selection == "best") {
    if(verbose) message("Best Model Selection: Selecting the single best learner...")
