---
title: "06. Machine Learning with Random Survival Forests"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{06. Machine Learning with Random Survival Forests}
...
```{r standalone-rf, eval=requireNamespace("randomForestSRC", quietly = TRUE)}
...
Plotting examples run only when both `randomForestSRC` and the optional `ggplot2` package are installed.


```{r plot-standalone, fig.align='center', eval=has_rfsrc && has_ggplot2}
...
```{r eval-standalone, fig.align='center', fig.height=4, fig.width= 9, eval=has_rfsrc && has_ggplot2}
...
```{r fit-models, results='hide', message=FALSE, warning=FALSE, eval=requireNamespace("randomForestSRC", quietly = TRUE)}
...
```{r plot-ensemble-benchmark, fig.align='center', fig.height=9, eval=has_rfsrc && has_ggplot2}
plot_benchmark(
...
Including flexible learners broadens the candidate library and may improve adaptation to nonlinear or non-proportional-hazards structure. Their contribution is determined by the selected cross-validated objective and should be evaluated on held-out data.
